	#a function that generates boosted decision stumps (BDS) for 2 attributes and any value of B
	#code for BDS3.r is identical to BDS2.r except for some additions to output test MSE
	BDS3 <- function (
		trainObject
	, 	testObject
	, 	trainLabel
	...
	RSS	<- function (X,n) {
			rss														<- sapply(	1:n
																			,	function(s)
																				sum((X - mean(X[which(X < X[s])]))^2)
																			+	sum((X - mean(X[which(X[s] <= X)]))^2)
																			);
			return(rss);
	};
	#INITIALISE DATA FRAMES
		...
		df.testLabel												<- 	data.frame(testLabel);	
	#PARAMETERS
...
#BEGIN BOOSTING
	...
	#CALCULATE MEAN VALUE OF THE trainLabels that exist in r1 and r2
		...
		f.hat.b														<- ifelse(df.trainObject[,j] < df.trainObject[s,j],r1.mean,r2.mean);
		f.hat.x 													<- f.hat.x + (lrate * f.hat.b);
		ri 															<- ri - (lrate * f.hat.b);	
#END BOOSTING
		}	
	#FINAL RESULT ON testObject		
		f.hat.b														<- ifelse(df.testObject[,j] < df.testObject[s,j],r1.mean,r2.mean);
		f.hat.x 													<- f.hat.x + (lrate * f.hat.b);
		
		test.MSE													<- mean((f.hat.x - df.testLabel)^2);		
	return(test.MSE);
